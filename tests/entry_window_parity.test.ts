...
// lockstep. Regression: the Loot Settings window shipped to index.html only would throw
// on /play the moment the auto-open fired (becoming party leader).
...
describe('entry window-panel parity', () => {
  ...
  it('includes the loot-settings window in both entries', () => {
    expect(windowPanelIds(read('index.html')).has('loot-settings-window')).toBe(true);
    expect(windowPanelIds(read('play.html')).has('loot-settings-window')).toBe(true);
  });
});
