...
describe('resolvePrewarmPolicy: desktop reproduces historical behavior', () => {
...
describe('resolvePrewarmPolicy: phone with parallel compile', () => {
  const p = resolvePrewarmPolicy({ ...BASE, mobileProfile: true, asyncCompileSupported: true });
  ...
  it('caps budget, compile budget, and nearby views hard', () => {
    ...
    // never the 72 that would spike in a populated world.
  ...
  it('restricts the manifest to the keep-list', () => {
    ...
    // Divergence from upstream, deliberate: the eager geometry upload stays. It is
    // this repo's camera-rotation-hitch fix and uploads buffers the session needs
    // resident anyway.
    expect(prewarmEntryRuns('world.geometry-upload', p)).toBe(true);
    ...
    expect(prewarmEntryRuns('entities.player-archetypes', p)).toBe(false);
    expect(prewarmEntryRuns('textures.scene', p)).toBe(false);
    expect(prewarmEntryRuns('sky.biome-variants', p)).toBe(false);
    expect(prewarmEntryRuns('vfx.atlas', p)).toBe(false);
  });
  ...
  it('honors maxViewsMobile only when it is below the tier cap', () => {
...
describe('resolvePrewarmPolicy: phone WITHOUT parallel compile', () => {
...
describe('the keep-list is the minimal entry set', () => {
  it('contains exactly the entries needed to enter without a first-frame stall', () => {
    expect([...MOBILE_PREWARM_KEEP].sort()).toEqual(
      [
        'programs.compile',
        'render.settle-passes',
        'views.nearby',
