...
describe('the World Market — the Merchant', () => {
  ...
  it('opens the same shared market at the Highwatch auctioneer (auctioneer_voss)', () => {
    const sim = makeWorld();
    const voss = [...sim.entities.values()].find((e) => e.templateId === 'auctioneer_voss');
    expect(voss).toBeTruthy();
    const shopper = sim.addPlayer('warrior', 'Shopper');
    const e = sim.entities.get(shopper)!;
    e.pos.x = voss!.pos.x;
    e.pos.z = voss!.pos.z;
    e.pos.y = groundHeight(e.pos.x, e.pos.z, sim.cfg.seed);
    e.prevPos = { ...e.pos };
    // Standing at the second auctioneer opens the World Market (non-null snapshot).
    expect(sim.marketInfoFor(shopper)).not.toBeNull();
  });
  ...
  it('keeps listings and collection items whose item id is no longer known', () => {
    ...
    // via the public serialize path, since the collection map is sim-private)
