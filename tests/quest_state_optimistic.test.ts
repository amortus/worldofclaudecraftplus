...
// Talking to an NPC and turning in a quest that gates a follow-up quest should
// immediately show the follow-up in the same dialog session rather than requiring
// the player to close and reopen the dialog. Online, `turnInQuest` is a
// fire-and-forget wire command: the questLog/questsDone mirror only updates once
// the next server snapshot round-trips, so a naive `questState()` read for the
// follow-up quest right after the turn-in click would still see the prerequisite
// as not-done.
describe('optimisticQuestState', () => {
  ...
  it("does not mutate the caller's authoritative questsDone set", () => {
    const questLog = new Map<string, QuestProgress>([
      ['q_wolves', { questId: 'q_wolves', counts: [8], state: 'ready' }],
    ]);
    const questsDone = new Set<string>();
    ...
    optimisticQuestState('q_greyjaw', questLog, questsDone, pendingQuestCommands, 5);
    expect(questsDone.size).toBe(0);
  });
