...
describe('traversal constants mirror the real movement constants', () => {
  ...
  it('matches the ground-snap base sim.ts absorbs without falling', () => {
    const base = num(
      simSrc,
      /const maxStepDown = ([\d.]+) \+ run \* MAX_CLIMB_SLOPE;/,
      'the maxStepDown base in sim.ts',
    );
    expect(base).toBe(GROUND_SNAP_BASE);
    ...
    expect(MAX_STEP_HEIGHT).toBe(base + RUN_SPEED * DT * PLAYER_MAX_CLIMB_SLOPE);
  });
