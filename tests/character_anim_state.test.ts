...
// A three.js SkinnedMesh blends toward BIND POSE (arms out, the T-pose) by
// however much the summed effective weight of the mixer's scheduled actions
// falls SHORT of 1: the PropertyMixer blends the deficit back toward the bind
// transform. These helpers are the pure half of the watchdog, and they catch
// only the FULL latch (a rig with nothing driving it at all); a partial deficit
// is deliberately out of scope, see POSE_DRIVE_MIN_WEIGHT. The live-mixer half
// is tests/character_tpose_repair.test.ts.
...
describe('drivesPose', () => {
  ...
  it('rejects a scheduled action whose fade-out has completed', () => {
    ...
    expect(drivesPose(driving(POSE_DRIVE_MIN_WEIGHT / 2))).toBe(false);
  });

  it('rejects nothing at all', () => {
    expect(drivesPose(null)).toBe(false);
    expect(drivesPose(undefined)).toBe(false);
  });
});
...
describe('scanAnimRepair', () => {
  ...
  it('repairs on the third consecutive starved frame, not before', () => {
    ...
    for (let i = 0; i < ANIM_REPAIR_FRAMES; i++) {
      const scan = scanAnimRepair(starvedFrames, [driving(0)], false);
      starvedFrames = scan.starvedFrames;
      fired.push(scan.repair);
    }
  ...
  it('resets the counter as soon as an action drives the pose again', () => {
    const scan = scanAnimRepair(ANIM_REPAIR_FRAMES - 1, [driving(1)], false);
    expect(scan.starvedFrames).toBe(0);
    expect(scan.repair).toBe(false);
  });
  ...
  it('writes into a caller-owned result so the per-frame path never allocates', () => {
    // visual.ts runs this once per rig per frame; a fresh object per rig is
    // measurable GC churn on the phone tier, so the scratch must be reused.
    const scratch = { starvedFrames: 0, repair: false };
    const first = scanAnimRepair(0, [driving(0)], false, scratch);
    expect(first).toBe(scratch);
    expect(scratch.starvedFrames).toBe(1);
    const second = scanAnimRepair(scratch.starvedFrames, [driving(1)], false, scratch);
    expect(second).toBe(scratch);
    expect(scratch).toEqual({ starvedFrames: 0, repair: false });
  });
});
...
describe('desiredBaseState', () => {
  it('never asks for walkBack when the loaded rig has no such action', () => {
    ...
    expect(desiredBaseState({ ...backwards, speed: 7 }, false)).toBe('run');
  });
...
describe('locomotionTimeScale', () => {
  ...
  it('returns null for every non-locomotion pose', () => {
    for (const state of ['idle', 'cast', 'swim', 'sit', 'jump'] as const) {
