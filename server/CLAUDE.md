...
- **New WS/loop-side behavior** is a sibling module, never another `GameServer`/`main.ts`
  ...
  `createWsAuth`, `moderation_service.ts`). `deeds_board.ts` (pure, IO-free) versus
  `deeds.ts` (DB+HTTP shell) is the same split for REST domains.
...
| `daily_rewards.ts`/`daily_rewards_db.ts` | the plain daily login reward: a daily spin, rotating tasks, a points ledger and the day's leaderboard. No money and no gate beyond moderation; participation bans are WRITTEN in `moderation_db.ts` (`setDailyRewardsBan`, permanent or timed via `durationHours`, recorded in the moderation audit; `tests/moderation_db.test.ts`), this pair owns only the eligibility read (`banForAccount`, `tests/daily_rewards_ban_db.test.ts`) |
