...
// above, from `itemSourceLevel` (item_source_level.ts), the level of the content
// the piece actually drops or is quested from. That keeps a level-6 rare gating
// at level 6 (it is at-level loot, not a twink item) instead of a flat quality
// band that both strands at-level drops above their source and under-gates
// higher-level rares below where they actually drop. Items with no derivable
// source (vendor stock, starter gear, synthetic/test items) fall back to a
// per-quality band.
//
...
// the sim equip path AND the HUD item tooltip, so it stays host-agnostic and is
// unit-tested directly.

import { itemSourceLevel } from './item_source_level';
