// The Nightbloom (level 20), ported from upstream `src/sim/content/nightbloom.ts`.
//
// A realm that is dreaming, in the WEST grid column directly north of the
// Palmreach: violet downs under a luminous lavender sky where a sleeping world
// hangs among the clouds, and the namesake flowers glow in the dream-light. The
// lantern village of Moonrest, the round Moonwell tarn, Gloamfield's flower
// downs, the Standing Vigil stone circle where the hovering nightkin keep their
// watch, and the Sleepless Barrow in the far north. Walked into from the jungle
// up the Nightgate (the southern border pass at southPassX -330).
//
...
//   - `PortalDef` has no equivalent here, and upstream's `NIGHTBLOOM_PORTALS` is
//     empty anyway: this realm is walked into over the Nightgate.
//   - `MobTemplate.componentTags` (on the grazer and the strider) is an upstream
//     corpse-harvest surface we do not have, so it is dropped rather than faked.
//   - The strider's `family` is remapped onto our 11-member `MobFamily`
//     (reptile -> beast); every value used here already has a rig in
//     `src/render/characters/`, so this pack adds no new art.
//   - Upstream's separate `NIGHTBLOOM_QUEST_CAMPS` (the barrow wights) is folded
//     into the tail of `NIGHTBLOOM_CAMPS` below, keeping its draw order last.
...
//     cited at the site.
//
// Camps are authored the way upstream wrote them, as scatter (`center`/`radius`/
// `count`, no `positions`). Freezing them to exact spots, the way the rest of
// this pack is frozen, is a later central pass and not this file's job.
...
export const NIGHTBLOOM_ROADS: { x: number; z: number }[][] = [
  [
    ...
    { x: -368, z: 1382 },
    { x: -370, z: 1420 },
  ], // the Nightgate to Moonrest
  [
    { x: -370, z: 1420 },
    { x: -334, z: 1402 },
    { x: -308, z: 1388 },
  ], // Moonrest to the Moonwell's shore
  [
    { x: -370, z: 1420 },
    { x: -408, z: 1452 },
    { x: -432, z: 1480 },
  ], // Moonrest to Gloamfield
  [
    { x: -370, z: 1420 },
    { x: -332, z: 1462 },
    { x: -298, z: 1508 },
    { x: -276, z: 1532 },
  ], // Moonrest to the Standing Vigil
  [
    { x: -370, z: 1420 },
    { x: -366, z: 1500 },
    { x: -362, z: 1570 },
    { x: -360, z: 1636 },
  ], // Moonrest to the Sleepless Barrow
  [
    { x: -360, z: 1636 },
    { x: -356, z: 1700 },
    { x: -350, z: 1760 },
    { x: -348, z: 1816 },
  ], // the Barrow to the gold road, EAST around the Barrowmere (upstream says
  // "west": the road runs x -356..-348 against the mere at x -336, i.e. -x, and
  // -x is EAST here)
  [
    { x: -280, z: 1550 },
    { x: -240, z: 1546 },
  ], // the Standing Vigil to the Palewater's shore
  [
    { x: -420, z: 1480 },
    { x: -470, z: 1514 },
  ], // Gloamfield to the sunset shore
];
...
export const NIGHTBLOOM_MOBS: Record<string, MobTemplate> = {
  ...
  // quest camps at the TAIL of NIGHTBLOOM_CAMPS (draw-order rule).
  barrow_wight: {
    id: 'barrow_wight',
    name: 'Barrow Wight',
    minLevel: 20,
...
export const NIGHTBLOOM_QUESTS: Record<string, QuestDef> = {
  q_nb_road_of_lanterns: {
    ...
    // "Moonrest lies north": verified, not translated. Sorrel stands at x -388
    // z 1284 and Moonrest sits at x -370 z 1420, i.e. +z with barely any x
    // offset, and +z is NORTH here (bearing 352, so due north).
  ...
  q_nb_eyes_on_the_vigil: {
    ...
    // "east of here" upstream, NORTHWEST here: Lira speaks from x -372 z 1417
    // and the Standing Vigil ring stands at x -272 z 1538, i.e. +x and +z, and
    // +x is WEST here (bearing 320, so northwest).
    text: 'Something has the striders bold and the herds uneasy, $N, and I cannot read it in the flowers. Cassian can read it in the sky. He keeps his observatory camp by the Standing Vigil northwest of here, where the nightkin drift among the stones. Find him, and ask what the stars are saying.',
    // "leans north toward the barrow" upstream, NORTHEAST here: Cassian charts
    // from x -278 z 1548 and the Sleepless Barrow lies at x -360 z 1650, i.e.
    // -x and +z, and -x is EAST here (bearing 39, so northeast).
    completionText:
      'Lira sent you? Then the gardens feel it too. Sit by the glass a moment, $N. The stars have been restless for a month, and every chart I draw leans northeast toward the barrow.',
    objectives: [
      {
        type: 'interact',
...
// Upstream's scatter camps, verbatim: `center`/`radius`/`count` with no exact
// `positions`. Freezing them to spots captured at the live seed (the rule the
// rest of this pack follows, so world generation draws no new rng) is a later
// central pass, not this file's.
export const NIGHTBLOOM_CAMPS: CampDef[] = [
  { mobId: 'moonfleece_grazer', center: { x: -436, z: 1466 }, radius: 12, count: 4,
    positions: [{ x: -429.29, z: 1474.2 }, { x: -441.4, z: 1459.36 }, { x: -439.01, z: 1473.72 }, { x: -444.94, z: 1460.35 }] },
  { mobId: 'moonfleece_grazer', center: { x: -320, z: 1446 }, radius: 10, count: 3,
    positions: [{ x: -318.44, z: 1453.4 }, { x: -318.54, z: 1451.29 }, { x: -325.43, z: 1451.55 }] },
  { mobId: 'gloam_strider', center: { x: -410, z: 1522 }, radius: 10, count: 3,
    positions: [{ x: -412.93, z: 1527.63 }, { x: -410.59, z: 1514.98 }, { x: -407.99, z: 1524.09 }] },
  { mobId: 'gloam_strider', center: { x: -240, z: 1402 }, radius: 10, count: 3,
    positions: [{ x: -235.13, z: 1401.81 }, { x: -231.83, z: 1399.5 }, { x: -245.01, z: 1405.51 }] },
  { mobId: 'nightkin_stargazer', center: { x: -272, z: 1538 }, radius: 8, count: 3,
    positions: [{ x: -275.53, z: 1536.21 }, { x: -273.82, z: 1534.17 }, { x: -274.32, z: 1536.92 }] },
  { mobId: 'barrow_king', center: { x: -360, z: 1650 }, radius: 5, count: 1,
    positions: [{ x: -357.39, z: 1648.82 }] },
  // Upstream's separate NIGHTBLOOM_QUEST_CAMPS (barrow wights on the grave
  // rows), folded in here at the TAIL so their draw order stays last.
  { mobId: 'barrow_wight', center: { x: -326, z: 1660 }, radius: 9, count: 3,
    positions: [{ x: -325.77, z: 1656.47 }, { x: -323.36, z: 1651.58 }, { x: -320.53, z: 1664.33 }] },
  { mobId: 'barrow_wight', center: { x: -382, z: 1636 }, radius: 9, count: 3,
    positions: [{ x: -384.02, z: 1629.46 }, { x: -389.7, z: 1636.52 }, { x: -373.71, z: 1633.75 }] },
];
