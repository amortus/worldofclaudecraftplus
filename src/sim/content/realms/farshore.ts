// The Farshore (levels 3-7), ported from upstream `src/sim/content/farshore.ts`.
//
// A small island in the starter sea beside Eastbrook Vale, all but surrounded by
// open ocean and joined to the mainland only by the Ferrywalk: a thin natural
// sandbar causeway from the vale's shore to the island's Landing, walked on foot
// (no teleport). The island is under siege. Rifts, the townsfolk call them "the
// breaks", tear open across the Farshore without warning and spill monsters onto
// the land; the fishing town of Gullhaven has become a redoubt, and its people
// hold their shore against whatever pours through. The break MECHANIC (the
// auto-generated dungeon portals themselves) is a separate system; this module
// builds the ISLAND around it: the besieged town, the defender cast who know
// exactly what is happening, and the break-spawned creatures that already made it
// through. Biome 'vale': the island shares the vale's sky, palette and song.
//
...
//   - `riftPortalEligible` / `riftTierWeights` are not fields of our ZoneDef. Our
//     rift rotation is DERIVED (`riftEligibleZones`, data.ts: any zone whose level
//     band reaches the cap), and this band tops out at 7, so the isle simply is
//     not rift-eligible here.
//   - POI records carry no `id` here; our `ZoneDef.pois` is `{x, z, label}`.
//   - `PortalDef` has no equivalent here and upstream's `FARSHORE_PORTALS` is
//     empty anyway: the isle is walked into over the Ferrywalk.
//   - `MobTemplate.componentTags` (the Void Stalker's hide/fang harvest tags) is
//     an upstream corpse-harvest surface we do not have, so it is dropped rather
//     than faked. The mobs' `family` is remapped onto our 11-member `MobFamily`
//     (burrower -> kobold, which is the rig upstream's own comment names for the
//     wretches); every value used here already has a rig in
//     `src/render/characters/`.
//   - `EscortDef` + `objectives[].type: 'escort'`: we have no escort system, so
//     `q_fs_bram_come_home`, upstream's `FARSHORE_ESCORTS` (`esc_fs_bram`) and its
//     escortee mob `fisher_bram` are NOT ported. That quest paid `itemRewards: {}`,
//     so no reward item had to move to a neighbour. Its giver Frightened Nell is
//     kept as a flavour NPC with an empty `questIds`, exactly as upstream wrote
//     her otherwise. Nothing else in the zone requires that quest.
//   - The ISLE_* terrain (the island landmass and the Ferrywalk sandbar) lives in
//     upstream's `world.ts` and is NOT ported: only the content rect is here, so
//     the isle currently sits on whatever our own terrain generates for it.
...
//     noted at each site.
//
// Camps are left as upstream authored them (scatter camps: `center`/`radius`/
// `count`, no `positions`). Freezing them to exact spots, the way `willowfen.ts`
// and `palmreach.ts` carry theirs, is a later central pass.
...
export const FARSHORE_ZONE: ZoneDef = {
  ...
  lakes: [
    ...
    // Upstream's placement rationale, kept as they wrote it; it describes their
    // ISLE_* terrain, which is not ported here.
...
export const FARSHORE_ROADS: { x: number; z: number }[][] = [
  [
    ...
    { x: 238, z: 12 },
    { x: 252, z: 15 },
  ], // the Ferrywalk causeway: the vale's shore point (x 150) out to the Landing
  // (x 252), i.e. +x, and +x is WEST here, so the isle lies west of the vale
  [
    { x: 250, z: 14 },
    { x: 276, z: 42 },
    { x: 305, z: 68 },
...
export const FARSHORE_NPCS: Record<string, NpcDef> = {
  ...
  // Frightened Nell gave the escort quest upstream (q_fs_bram_come_home), which
  // this port cannot carry. She keeps her post and her greeting as a flavour NPC.
  fisher_nell: {
    id: 'fisher_nell',
...
export const FARSHORE_QUESTS: Record<string, QuestDef> = {
  ...
  q_fs_hold_the_riftfields: {
    ...
    turnInNpcId: 'warden_coalfast',
    // "West of town" (upstream "East of town"): the Riftfields sit at x 434,
    // z 58 against Gullhaven at x 305, z 70, i.e. +x, and +x is WEST here
    // (bearing 265, nearest rose point west).
    text: 'West of town the grain rows have gone to wrack, and the wretches that came through the Riftfields break now pick them clean. My people cannot tend a field they cannot stand in, $N. Cull ten of the wretches and give the farmers back their ground.',
    completionText:
  ...
  q_fs_the_three_bells: {
    ...
    // "the northwest strand" (upstream "the south strand"): the second bell
    // stands at x 318, z 94 against Gullhaven at x 305, z 70, i.e. +z and a
    // little +x, and +z is NORTH with +x WEST (bearing 332, nearest rose point
    // northwest). Upstream was wrong on its own axes here too: z 94 is north of
    // the town on their +z-north map as well.
  ...
  q_fs_song_before_the_break: {
    ...
    // "southwest of town" (upstream "southeast of town"): the Watch Meadow sits
    // at x 375, z -5 (Ollun himself at x 372, z 2) against Gullhaven at x 305,
    // z 70, i.e. +x and -z, and +x is WEST with -z SOUTH (bearing 223 to the
    // meadow, 225 to Ollun, nearest rose point southwest).
    text: 'There is a man who hears the breaks before they open. Riftwatch Ollun: a scholar, or a madman, and lately I cannot afford the difference. He keeps his vigil at the Watch Meadow, up the road southwest of town. Find him, $N, and ask him what the island is about to do to us next.',
    completionText:
  ...
  q_fs_moss_and_mending: {
    ...
    // "the southwest reaches" (upstream "the east reaches"): the riftspawn hold
    // camps at x 452, z 66 and x 388, z -44 (centroid x 420, z 11) against
    // Mender Saul at x 312, z 78, i.e. +x and -z, and +x is WEST with -z SOUTH
    // (bearing 238, nearest rose point southwest).
    text: 'The salt moss that grows along the tideline is the best wound-packing I know, and the riftspawn have claimed every stretch of shore it grows on. They carry tufts of it snagged on their hides, of all things. Clear six of them off the southwest reaches, $N, and pull me four good handfuls of moss from what they have trampled through.',
    completionText:
  ...
  q_fs_the_great_break: {
    ...
    giverNpcId: 'riftwatch_ollun',
    turnInNpcId: 'warden_coalfast',
    // "walks northeast" (upstream "walks north"): the horror's camp at the
    // Sundered Cliffs is x 402, z -78 (the POI x 402, z -72) and Gullhaven, the
    // town it would walk toward, is x 305, z 70, i.e. -x and +z, and -x is EAST
    // with +z NORTH (bearing 33 from the cliffs, nearest rose point northeast).
    text: 'Every song this island sings ends on the same low note, and it comes from the Sundered Cliffs. Something came through the great break there, $N, something the cliffs themselves cracked open to admit, and it is still growing. If it walks northeast, no bell will matter. Take a friend, take two, and end it. Then tell Coalfast the tune has changed.',
    completionText:
...
// Veiled Hollow convention. Upstream's 'q_fs_bram_come_home' is absent: it is
// the escort quest this port cannot carry.
...
// places these itself. Freezing them (as willowfen.ts and palmreach.ts carry
// theirs, to keep the post-worldgen rng cursor bit-identical) is a later pass.
export const FARSHORE_CAMPS: CampDef[] = [
  ...
  { mobId: 'breach_wretch', center: { x: 430, z: 44 }, radius: 12, count: 4, // the Riftfields
    positions: [{ x: 428, z: 46.58 }, { x: 435.43, z: 42.97 }, { x: 423.84, z: 41.75 }, { x: 423.02, z: 41.32 }] },
  { mobId: 'breach_wretch', center: { x: 400, z: 96 }, radius: 11, count: 3, // the north downs
    positions: [{ x: 397.48, z: 96.1 }, { x: 406.35, z: 100.59 }, { x: 407.3, z: 92.36 }] },
  { mobId: 'riftspawn', center: { x: 452, z: 66 }, radius: 11, count: 3, // deep in the Riftfields
    positions: [{ x: 445.04, z: 73.04 }, { x: 453.24, z: 58.85 }, { x: 443.66, z: 71.34 }] },
  { mobId: 'riftspawn', center: { x: 388, z: -44 }, radius: 11, count: 3, // above the Sundered Cliffs
    positions: [{ x: 379.42, z: -42.22 }, { x: 382.17, z: -52.86 }, { x: 392.98, z: -44.22 }] },
  { mobId: 'void_stalker', center: { x: 418, z: -30 }, radius: 12, count: 2, // the cliff approach
    positions: [{ x: 411.95, z: -32.34 }, { x: 425.58, z: -36.4 }] },
  { mobId: 'void_stalker', center: { x: 462, z: 20 }, radius: 11, count: 2, // the far reach
    positions: [{ x: 462.99, z: 10.61 }, { x: 461.47, z: 25.55 }] },
  { mobId: 'sundered_horror', center: { x: 402, z: -78 }, radius: 6, count: 1, // the cliffs' great break
    positions: [{ x: 405.11, z: -77.33 }] },
];
...
export const FARSHORE_OBJECTS: GroundObjectDef[] = [
  {
    ...
    // Landing point, the strand northwest of Gullhaven (x 318, z 94 against the
    // town at x 305, z 70), the Riftfields shore.
    positions: [
      { x: 256, z: 0 },
      { x: 318, z: 94 },
      { x: 442, z: 64 },
