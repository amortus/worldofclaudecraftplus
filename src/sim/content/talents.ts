...
// straight to computeTalentModifiers, which trusts the apply-time gate; but the load
// path never ran validateAllocation, so a stale, level-downed, or tampered save could
// grant over-budget / prereq-broken / gated stats and abilities.
//
// This rebuilds the allocation deterministically: walk the tree top-down, class tree
// first then the chosen spec, in row/col order (the order defaultBuild uses), refilling
// each node up to its persisted rank but never past a point where validateAllocation
// would reject the build. Because prereqs and pointsGates only reference rows above, a
// top-down fill satisfies them by construction, and the running budget check clamps the
// total to availablePoints. On an already-valid, in-budget allocation this is the
// identity (every persisted rank validates at each step), so honest saves load
// byte-identically.
export function repairAllocation(
  ...
  // FIRST_TALENT_LEVEL (availablePoints === 0) a spec is illegal (it would still grant
  // the signature ability + mastery passive), matching the apply-time gate.
  const spec =
