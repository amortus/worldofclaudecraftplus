...
export function recalcPlayerStats(
  ...
  for (const slot of EQUIP_SLOTS) {
    ...
    // or armor until the character reaches its required level. This only arises
    // for a character loaded wearing gear equipped before the level gate
    // existed; the equip path blocks equipping over-level gear.
  ...
  // Render-only mirror of the full worn set, copied so a later mutation of the owning
  // PlayerMeta.equipment never aliases into the entity. Synced in the identity wire
  // (terse `eq`) for the inspect-another-player window.
