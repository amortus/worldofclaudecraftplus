...
// boss's hate table, participant HP scaling, and the personal-loot roller. The
// SCHEDULER state and the spawn primitive live on `Sim` (it needs createMob/
// addEntity/groundPos), which drives this module each tick.
//
// Our fork keeps game logic inline in sim.ts (no SimContext), so unlike upstream
// these helpers take their dependencies (the entities/players maps, the shared Rng)
// as explicit arguments rather than through a ctx object. They stay a pure sibling
// module like threat.ts: no DOM/Three imports, no Math.random/Date.now. Randomness
// is the passed Rng only, and the lockout boundary is the host lockout clock plus
// raid-reset instant (the same pair the dungeon raid lockouts use; the host wall
// clock on the server, the sim clock offline). The personal-loot roller draws rng in
// a FIXED order (contributors sorted by entityId, loot entries in array order) so the
// parity gate's rng draw-order log stays stable.
...
export const WORLD_BOSSES: readonly WorldBossDef[] = [
  {
    ...
    // BALANCE (maintainer review): 40k solo, +5k per extra participant, up to 1M. The
    // per-player step is deliberately gentle: scaleWorldBossHp adds each joiner's delta
    // to CURRENT hp too (real health, not a heal), so a steep step made the bar visibly
    // refill as a raid trickled in and read as "he takes no damage". 5k/head keeps the
    // fight scaling without stalling it.
...
// name resolver can spot it and localize it as a mob name. The world boss is a genuine
// raid lockout: the SAME `meta.raidLockouts` entry that renders the countdown is what
// the eligibility gate reads, so the displayed timer is exactly the loot lockout, and
// it resets on the same boundary as the raids.
...
// (the host wall clock on the server and the sim clock offline, never a
// deterministic-tick value).
...
// next raid-reset instant, the same boundary the dungeon raids reset on). Called from
// lootCorpse when a personal world-boss slot is actually taken, NOT at kill/roll time.
// This single write is both the eligibility gate (isWorldBossLootEligible) and the
...
// boss's threat.
export function worldBossContributors(
...
export function worldBossLootContributors(
  entities: Map<number, Entity>,
  players: Map<number, PlayerMeta>,
  mob: Entity,
): PlayerMeta[] {
...
export function scaleWorldBossHp(
  entities: Map<number, Entity>,
  players: Map<number, PlayerMeta>,
  boss: Entity,
  def: WorldBossDef,
  ...
  const participants = worldBossContributors(entities, players, boss).length;
  ...
  if (target > boss.maxHp) {
    ...
    // becomes ~58% of 48k). That is intentional ("real health, not a heal").
...
// there is no questId gating and no per-entry copper here; a world-boss loot table
// must not use those fields.
export function rollWorldBossLoot(
  rng: Rng,
  mob: Entity,
  ...
  // actually LOOTS a personal slot (lootCorpse): a contributor who dies or never
  // reaches the corpse inside the loot window keeps their lockout and can try again at
  // the next spawn. The corpse window (WORLD_BOSS_CORPSE_SECONDS) never overlaps the
  // spawn cadence, so at most one corpse is ever lootable at a time.
  for (const meta of contributors) {
    if (!isWorldBossLootEligible(meta, mob.templateId, lockoutNowMs)) continue;
