...
// server (the inline market in src/sim/sim.ts, which filters + paginates
// authoritatively) and the client filter chrome (src/ui/market_filters.ts re-exports
// the option lists) share one definition and can never drift. Moving filtering
// server-side is what lets a player page through and filter the WHOLE market, not
// just the first wire window.
