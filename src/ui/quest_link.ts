...
//
// ENCODING IS GUARDED. The parser matches a bounded id charset, and a token it
// cannot match is not dropped: it survives parseChatSegments as a text segment, so
// the player (and every recipient) reads the literal "[[i:...]]" source. Every
// production encode site therefore goes through tryEncodeQuestLink /
// tryEncodeItemLink, which answer "is this id safe to encode" once, here, beside the
// regex that decides it. The raw encoders stay exported for the parser-agreement test.
...
// The same id charset as CHAT_LINK_RE, anchored, for callers that build a token from
// an id they did not choose (content data). This lives beside the regex so the two
// can never answer the question differently, and it is the ONE place the charset is
// written down.
...
// The RAW encoders below interpolate whatever id they are handed, charset and all, so
// a caller that skipped the guard would ship source text to the player. Production
// code uses the tryEncode* pair instead; these stay exported so the test suite can
// mint a deliberately unmatchable token and prove the parser rejects it, which is what
...
/** The chat token for `questId`, or null when the parser's charset would not match
 *  the id. Returning null rather than a doomed token is what lets the guard live at
 *  the encoder instead of being restated (or forgotten) at each call site: a caller
 *  with a fallback takes it, a caller without one returns early, and neither has to
 *  know the charset. */
