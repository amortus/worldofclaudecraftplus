...
//               auras `breaksOnDamage` at their inline emit sites (the incapacitate/polymorph
//               apply sites in src/sim/sim.ts); a swing would shatter the CC, so an ability
 ...
 * "Invalid attack target." toast on every cast (the sim raises that in startAutoAttack,
 * src/sim/sim.ts). Only mobs carry `hostile:true` (players/NPCs default false) and the
 * server mirrors the flag onto the wire, so the same read holds for the offline Sim and
 * the online ClientWorld.
 *
