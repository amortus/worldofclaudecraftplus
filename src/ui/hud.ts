...
export class Hud {
  ...
  // placePopupAt was given: measure the real rendered box and pull it back
  // on-screen. The unit context menu with the 40px mobile-floor items can exceed a
  // fixed reserve on a short landscape phone; getBoundingClientRect reflects the
  // laid-out box (so it is reliable even on the first open, where an offset read
  // can still be stale), and this only ever moves the popup UP/LEFT, never past the
  // top/left edge. Call it AFTER the popup is display:block so the box measures.
  private keepPopupOnScreen(el: HTMLElement): void {
    ...
    if (typeof requestAnimationFrame === 'function') requestAnimationFrame(clamp);
  }
  ...
  // Like itemIcon but non-blocking: if the procedural icon is not cached yet, paint a
  // transparent placeholder tagged for idle fill (fillAsyncIcons) instead of encoding it
  // synchronously. For long item lists (market) so the open is a cheap paint, not a
  // toDataURL burst. Cached / image-art icons still return their real src immediately.
  private itemIconAsync(item: ItemDef): string {
    const q = item.quality ?? 'common';
    const ready = iconDataUrlIfReady('item', item.id);
    if (ready) return `<img class="item-icon q-${q}" src="${ready}" alt="" draggable="false">`;
    return `<img class="item-icon q-${q}" src="${ICON_PLACEHOLDER}" data-async-item="${esc(item.id)}" alt="" draggable="false">`;
  }

  // Fill the placeholder icons left by itemIconAsync in a just-rendered container,
  // encoding each uncached icon in the idle-capped icon-warm pump and swapping its <img>
  // src in when ready. A closed / re-rendered panel simply no-ops the swap.
  private fillAsyncIcons(container: HTMLElement): void {
    const imgs = container.querySelectorAll<HTMLImageElement>('img[data-async-item]');
    if (imgs.length === 0) return;
    const tasks: Array<() => void> = [];
    imgs.forEach((img) => {
      const id = img.dataset.asyncItem;
      if (!id) return;
      tasks.push(() => {
        img.src = iconDataUrl('item', id);
        delete img.dataset.asyncItem;
      });
    });
    warmIcons(tasks);
  }
  ...
  private renderDelveBoard(focus = false): void {
    ...
    if (tab === 'shop') {
      ...
      this.fillAsyncIcons(el); // idle-fill the shop's placeholder item icons
    } else {
  ...
  openInspect(pid: number): void {
    ...
    el.innerHTML =
      ...
      `<div class="inspect-equip-list" id="inspect-equip-list"></div>` +
      `</div>`;
    hydratePortraits(el);
    // Worn gear, resolved fully client-side from the static ITEMS table; only the
    // slot->id map (the `eq` identity field) crosses the wire. Read-only: another
    // player's gear, so no unequip/drag affordances.
    const equipList = el.querySelector('#inspect-equip-list');
    if (equipList) {
      let any = false;
      for (const slot of EQUIP_SLOTS) {
        const itemId = e.equippedItems[slot];
        const item = itemId ? ITEMS[itemId] : undefined;
        if (!item) continue;
        equipList.appendChild(this.buildInspectSlotRow(slot, item));
        any = true;
      }
      if (!any) {
        const empty = document.createElement('div');
        empty.className = 'inspect-equip-empty';
        empty.textContent = t('itemUi.equipment.empty');
  ...
  // One read-only equipment row for the inspect window: item icon, the equipped item
  // name (quality-tinted) and its slot label, with the item tooltip on hover. View-only
  // (another player's gear), so no unequip / drag affordances.
  private buildInspectSlotRow(slot: EquipSlot, item: ItemDef): HTMLElement {
    const row = document.createElement('div');
    row.className = 'inspect-equip-slot';
    const qColor = QUALITY_COLOR[item.quality ?? 'common'] ?? '#fff';
    row.innerHTML =
      `${this.itemIcon(item)}<div class="inspect-equip-info">` +
      `<div class="inspect-equip-item" style="color:${qColor}">${esc(itemDisplayName(item))}</div>` +
      `<div class="inspect-equip-slotname">${esc(itemSlotName(slot))}</div></div>`;
    this.attachTooltip(row, () => this.itemTooltip(item, false));
    return row;
  }
