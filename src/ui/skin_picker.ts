...
// neighbour's mouseenter: hovering the swatch next to the selected one reverted
// the preview instead of showing it. A row-level mouseleave does not fire on
// child-to-child transitions, so every swatch previews consistently while the
// revert still happens exactly when the pointer leaves the picker.
