...
//
// On phone-class browsers (worst on iOS WebKit, where Safari AND every app shell run the
...
// (a saved choice, or the auto default), every retry crashes the same way and the player
// is trapped in a load, crash, reload loop before any screen with a graphics control.
//
...
// - clearEntryProbe() removes it once entry has demonstrably survived (frames are
//   rendering), on the handled failure paths (a caught renderer error already surfaces
//   its own overlay), and when the page leaves the foreground (a backgrounded eviction
//   is NOT an entry crash and must never cost a tier).
...
//   tier down to retry with. The caller persists the lowered preset and pins it
//   (graphicsDefaultApplied) so the auto default cannot re-select a tier this device has
//   proven it cannot enter the world at.
//
...
// where the ladder stops, without hardcoding per-model assumptions the masked mobile GPU
// string cannot support.
//
// The caller (main.ts) gates both stamping and recovery on GFX.mobileProfile, so desktop
// boots never touch the probe; this module itself stays profile-agnostic and pure.
//
