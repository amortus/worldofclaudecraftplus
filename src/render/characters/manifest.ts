 ...
 * preloaded" synchronously when the resolved URL was never loaded. The live tier is set
 * by initGfxTier() inside the Renderer constructor, AFTER assets.ts froze its import-time
 * GFX best-guess. On low gfx, LOW_URL_ALIAS swaps one body GLB (rogue_hooded.glb ->
 * rogue.glb), so manifestUrlsForGraphics(false) is a STRICT subset of
 * manifestUrlsForGraphics(true). If the import-time guess is low but the renderer
 * resolves medium+, the very common mob_bandit body (rogue_hooded.glb, the humanoid
 * default AND the global mob fallback) is placed yet was never preloaded, crashing world
 * entry: the character-side twin of the prop preload P0. So preload the UNION across both
 * tiers, exactly as foliage.ts is immune by sourcing one frozen list for both.
 *
