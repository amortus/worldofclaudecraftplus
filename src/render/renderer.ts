...
export class Renderer {
  ...
  constructor(
    ...
    // drifting low over the dead land (not riding the camera). Anchored over the
    // zone's north-central ground at ~23u altitude so it hangs just overhead with
    ...
    this.naxx = new SkyLandmark(this.scene, {
      ...
      anchor: { x: 0, z: 1150 },
      spin: 0.01,
      clearance: 23, // hovers low above the fixed anchor's ground (was 90 -> 45 -> 23)
  ...
  pick(clientX: number, clientY: number): number | null {
    ...
    // so a click on the floating name (what a healer does to target a party member)
    ...
    for (const [id, v] of this.views) {
      ...
      // Overhead anchor (the +1.0 chat-bubble offset). Collapse the column to the
      // body point if the anchor is not safely in front of the camera: a point
      // behind the near plane projects to bogus screen coords that could steal an
      // unrelated click. Same guard the real nameplate path uses.
